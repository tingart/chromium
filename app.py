...
import asyncio, urllib.parse, urllib.error, os, time, json, tarfile, tempfile, shutil, urllib.request, secrets, hashlib, hmac, base64
...
async def authentication_middleware(request: Request, call_next):
    ...
    public = path in {"/login", "/logout", "/favicon.ico"}
...
async def start_browser_context():
    ...
    if not pages:
        ...
        pages.append(p)
    active_tab_index=0;mark_screen_dirty("browser-started")
...
async def update_settings(p:SettingsPayload):
    ...
    try:
        if context:await context.set_viewport_size(current_viewport())
    except:pass
...
async def new_tab():
    ...
    try:
        p=await context.new_page();pages.append(p);active_tab_index=len(pages)-1;asyncio.create_task(background_home(p));mark_screen_dirty("new-tab");return await tabs_route()
    except Exception as e:return {"status":"error","message":str(e)}
